...
def bskycalc(isky, filtcurv, lambdarr, xa=2000, xb=12000, dx=1., aeff=aeff, hplk=hplk, cvl=cvlcm, pixscale=pixscale, averfact=averfact):
...
def defbackposit():
    ...
    for cssband in cssbands:
        filterthr = np.loadtxt('./throughput/'+cssband+'.txt')
        backsky[cssband] = bskycalc(isky, filterthr, lambdarr)*pixscale**2
        bandpos[cssband] = bandposit(filterthr)
