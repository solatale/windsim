...
import configparser
import csstpkg_phutil_mp_uBgN as csstpkg
...
def simul_css(CataSect, _CssImg, cssbands, filtnumb, npi):
    ...
    for procedi,cataline in enumerate(CataSect, 1):
        ...
        for cssband, numb in zip(cssbands, filtnumb):
            ...
            outcatrowi = outcatrowi + [modmag, FluxMsr, FLuxErr, SNR]
            bandi = bandi + 1
    ...
    if IfTileCata == True:
        ...
        if IfBandSim == True:
            ...
            MagSim_Zero = {}
                ...
                # 'uB':-14.023682234980917,
                # 'g': -13.401045545401303,
                # 'gN':-13.500697072199138,
            ...
            # colnames = ['ID','Z_BEST']+list(itertools.chain(*namelists))+['Npix']

            LenCatTile = len(CatOfTile)
            print('Catalog Length: ',LenCatTile)

            # Output catalog for one tile
            OutCssCatName = 'Cssos_FluxSim_SNR_tile_'+str(sys.argv[1])+'_allin1_uBgN.txt'
