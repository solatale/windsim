...
def MagAB_Ener_Zero(cssband):
    ...
    thrput = np.loadtxt(thrghdir+cssband+'.txt')
    thrfine = interp(thrput, xmin=bandpos[cssband][0], xmax=bandpos[cssband][2], dx=1)
    ToBeInteg = thrfine[:,1]  # to be integrated
    Integ = np.trapz(ToBeInteg, x=thrfine[:,0], dx=1.)
    area = Integ  # fnu in ergs/Hz/s/cm^2
    magab0 = 2.5*math.log10(area)-48.6
    return magab0


def mag_ener(modsed, cssband, magzero):
    magzb0 = MagAB_Ener_Zero(cssband)
    thrput = np.loadtxt(thrghdir+cssband+'.txt')
    thrfine = interp(thrput, xmin=bandpos[cssband][0], xmax=bandpos[cssband][2], dx=1)
    sedfine = interp(modsed, xmin=bandpos[cssband][0], xmax=bandpos[cssband][2], dx=1)
    ToBeInteg = sedfine[:,1]*thrfine[:,1]*thrfine[:,0]**2
    flux = np.trapz(ToBeInteg, x=thrfine[:,0], dx=1.)/3e18
    magab = -2.5*math.log10(flux)+magzero
    return magab


def MagAB_Phot_Zero(cssband):
    """
...
def mag_phot(modsed, cssband, magzero):
    magzb0 = MagAB_Ener_Zero(cssband)
    thrput = np.loadtxt(thrghdir+cssband+'.txt')
    thrfine = interp(thrput, xmin=bandpos[cssband][0], xmax=bandpos[cssband][2], dx=1)
    sedfine = interp(modsed, xmin=bandpos[cssband][0], xmax=bandpos[cssband][2], dx=1)
    ToBeInteg = sedfine[:,1]*thrfine[:,1]*thrfine[:,0]**3
    flux = np.trapz(ToBeInteg, x=thrfine[:,0], dx=1.)/3e18
    magab = -2.5*math.log10(flux)+magzero
    return magab
