...
#datetag=`date +%Y%m%d`
datetagmd=`date +%m%d`
export LEPHAREWORK='/work/CSSOS/lephare_dev/sim2phtz_allin1_Yeu'
export baseworkdir='/work/CSSOS/filter_improve/fromimg/windextract'
export resultdir=$baseworkdir'/result1020'
echo "LePhare Work in "$LEPHAREWORK

tiles=''
tsleep=1
for tile in 065 #039 040 041 042 043 051 052 053 054 055 063 064 065 066 067 075 #076 077 078 079 087 088 089 090 091
    do
        simcatname="CssEu_FluxSim_SNR_tile_"$tile"_allin1.txt"
        echo 'Tile: '$tile
        python3 hst814simsed_phutil_mp_flux_allin1_CssEu.py $tile
        ls $simcatname >> $mergelist
        tmp=$tiles
        tiles=$tmp$tile
        # sleep $tsleep
    done
mergedfile="CssEu_FluxSim_SNR_tilemrg_"$tiles"_allin1_"$datetag".txt"
