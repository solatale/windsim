...
class CentrlPhot:
    ...
    def Bkg(self, idb='NA', debug=False, thresh=1.5, minarea=10, deblend_nthresh=32, deblend_cont=0.005, clean_param=1.0):
        ...
        if debug==True:
            print('bkg rms =', self.bkg.globalrms)
            ...
            vmax = np.max(bkgimg)
            # print(vmin, vmax)
            plt.imshow(self.bkg, interpolation='nearest', cmap='gray', origin='lower', vmin=vmin, vmax=vmax)
    ...
    def Centract(self, idt='NA', debug=False, thresh=1.5, err=None, minarea=10, deblend_nthresh=32, deblend_cont=0.005, clean_param=1.0):
        ...
        self.data_bkg = self.data - self.bkg
        if debug==True:
            print('Bkg.globalrms =', self.bkg.globalrms)


        objects, segarr = sep.extract(self.data_bkg, thresh, err=self.bkg.globalrms, minarea=minarea, deblend_nthresh=deblend_nthresh, deblend_cont=deblend_cont, clean_param=clean_param, segmentation_map=True)
        ...
        if debug == True:
            ...
            for i in range(len(objects)):
                kronrdets= sep.kron_radius(self.data_bkg, objects[i]['x'], objects[i]['y'], objects[i]['a'], objects[i]['b'], objects[i]['theta'], 3.0)[0]
                e = Ellipse(xy=(objects[i]['x'], objects[i]['y']), width=objects[i]['a']*kronrdets*CentrlPhot.kphotpar*2, height=objects[i]['b']*kronrdets*CentrlPhot.kphotpar*2, angle=objects[i]['theta'] * 180. / np.pi)
        ...
        if self.centobj is not np.nan:
            ...
            self.maskarr = copy.deepcopy(segarr)
            # convert maskarr setting central object with 1, other with 0
            self.maskarr[self.maskarr != censeg] = 0
            self.maskarr[self.maskarr == censeg] = 1
            # self.maskarr[segarr==0]=np.max(segarr)+1
            # self.maskarr[segarr==censeg]=0
            # self.maskarr[self.maskarr>0] = np.max(self.maskarr)+2
            # self.maskarr[self.maskarr==0] = 1
            # self.maskarr[self.maskarr>1] = 0

            self.data_bkg_masked = self.data_bkg  # *self.maskarr

            if debug==True:
                plt.imshow(self.maskarr, interpolation='nearest', cmap='gray', origin='lower')
                plt.title(idt+"'s Mask")
                plt.show()


    def KronR(self, idk='NA', debug=False):
        try:
            self.kronr, krflag = sep.kron_radius(self.data_bkg, self.centobj['x'], self.centobj['y'], \
                                             self.centobj['a'], self.centobj['b'], self.centobj['theta'], 3) #, mask=self.maskarr,maskthresh=0)
        ...
        if debug==True:
            ...
            # Plot Cleaned object
            fig, ax = plt.subplots()
            # m, s = np.mean(self.data_bkgmask), np.std(self.data_bkgmask)
            # ax.imshow(self.data_bkgmask, interpolation='nearest', cmap='gray', origin='lower')
            m, s = np.mean(self.data_bkg), np.std(self.data_bkg)
            ax.imshow(self.data_bkg, interpolation='nearest', cmap='gray', origin='lower', vmin=m-2*s, vmax=m+3*s)
            # plot an ellipse for each object
    ...
    def EllPhot(self, kronr, debug=False):
        self.centflux, centfluxerr, flag = sep.sum_ellipse(self.data_bkg_masked, self.centobj['x'], self.centobj['y'], self.centobj['a'], self.centobj['b'], self.centobj['theta'], CentrlPhot.kphotpar * kronr, subpix=1) #, mask=self.maskarr,maskthresh=0.0)
        ...
        self.rsserr = np.sqrt(self.centflux+npix*self.bkg.globalrms**2)
        if debug==True:
...
def septract(dataorig, id='NA', debug=False, thresh=2., minarea=5, deblend_nthresh=32, deblend_cont=0.005, clean_param=1.0):
    ...
    if debug == True:
        ...
        # plot an ellipse for each object
        for i in range(len(objects)):
            kronrdets= sep.kron_radius(data_bkg, objects[i]['x'], objects[i]['y'], objects[i]['a'], objects[i]['b'], objects[i]['theta'], 4.0)[0]
            e = Ellipse(xy=(objects[i]['x'], objects[i]['y']), width=objects[i]['a']*kronrdets*kphotpar*2, height=objects[i]['b']*kronrdets*kphotpar*2, angle=objects[i]['theta'] * 180. / np.pi)
            e.set_facecolor('none')
            e.set_edgecolor('red')
            ax.add_artist(e)
    ...
    maskarr = segarr
    maskarr[segarr==0]=np.max(segarr)+1
    maskarr[segarr==censeg]=0

    # flip maskarr and set central object with 1, other with 0
    maskarr[maskarr>0] = np.max(maskarr)+2
    maskarr[maskarr==0] = 1
    maskarr[maskarr>1] = 0
    data_bkgmask = data_bkg*maskarr

    if debug==True:
        plt.imshow(maskarr, interpolation='nearest', cmap='gray', origin='lower')
        plt.title(id+"'s Mask")
        plt.show()

    kronri, krflag = sep.kron_radius(data_bkg, objecti['x'], objecti['y'], objecti['a'], objecti['b'], objecti['theta'], 4.0) #, mask=maskarr,maskthresh=0)
    if debug==True:
    ...
    flux, fluxerr, flag = sep.sum_ellipse(data_bkgmask, objecti['x'], objecti['y'], objecti['a'], objecti['b'], objecti['theta'], kphotpar * kronri, subpix=1) #, mask=maskarr,maskthresh=0.0)
    ...
    if rsserr == 0:
        ...
        return flux, rsserr, objecti, kronri, maskarr


def septractSameAp(dataorig, object_det, kronr_det, mask_det=1, debug=False, annot='', thresh=2., minarea=5, deblend_nthresh=32, deblend_cont=0.005, clean_param=1.0):
    # extract objects using "sep" program.
    ...
    bkg0 = Background2D(data, (16, 16), filter_size=(3, 3),
    ...
    objs = objimg * seg0
    bkgimg = data - objs
    bkg = Background2D(bkgimg, (back_size,back_size), filter_size=(3, 3), sigma_clip=sigma_clip, bkg_estimator=bkg_estimator)
