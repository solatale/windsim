"""
Usage: python3 fluxsim2lephare_sampleselect.py $mergedsimmagfile 424
...
for i,aline in enumerate(simcat0):
    if (((aline['SNR_g']>=5) and (aline['SNR_r']>=5) and (aline['SNR_i']>=5) and (aline['SNR_z']>=5))
            or (aline['SNR_r']>=7 and (aline['SNR_i']>=7)) or (aline['SNR_g']>=10) or (aline['SNR_r']>=10)
            or (aline['SNR_i']>=10) or (aline['SNR_z']>=10) or
            ((aline['SNR_g']**2+aline['SNR_r']**2+aline['SNR_i']**2+aline['SNR_z']**2)>=100)):
