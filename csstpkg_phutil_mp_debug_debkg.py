...
class CentrlPhot:
    ...
    def Bkg(self, idb='NA', debug=False, thresh=1.5, minarea=10, deblend_nthresh=32, deblend_cont=0.005, clean_param=1.0):
        ...
        self.masksrc = make_source_mask(self.data, nsigma=2, npixels=10, dilate_size=9)
        self.bkgmean, self.bkgmedian, self.bkgstd = sigma_clipped_stats(self.data, sigma=3.0, mask=self.masksrc)
        self.data_bkg = self.data - self.bkgmean

        if debug==True:
            print((self.bkgmean, self.bkgmedian, self.bkgstd))
            plt.imshow(self.masksrc, origin='lower')
            plt.show()
        ...
        masksrc2 = make_source_mask(self.data_bkg, nsigma=2, npixels=10, dilate_size=9)
        bkgmean2, bkgmedian2, bkgstd2 = sigma_clipped_stats(self.data_bkg, sigma=3, mask=masksrc2)
        if debug==True:
...
def septractSameAp(dataorig, stackphot, object_det, kronr_det, mask_det=0, debug=False, annot='', thresh=2., minarea=5, deblend_nthresh=32, deblend_cont=0.005, clean_param=1.0, sub_backgrd_bool=False):
    ...
    bkgmean, bkgmedian, bkgstd = sigma_clipped_stats(data, sigma=3.0, mask=stackphot.maskall)
