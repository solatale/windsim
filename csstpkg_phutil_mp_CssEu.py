...
def bskycalc(isky, filtcurv, lambdarr, xa=2000, xb=12000, dx=1., aeff=TelArea['i'], hplk=hplk, cvl=cvlcm, pixscale=0.075, averfact=averfact):
    bskycurv = curvemultiply(curvemultiply(isky, filtcurv), lambdarr)
    bsky = quadrat(bskycurv, xa, xb)*1e-8*aeff/hplk/cvl*averfact  # e-/arcsec^2
    # bskypix = quadrat(bskycurv, xa, xb)*1e-8*aeff/hplk/cvl*pixscale**2*averfact
    return bsky


def bandposit(filterthrarr):
    thrputfine = interp(filterthrarr, xmin=2000, xmax=12000, dx=10)
    effimax = np.max(thrputfine[:,1])
    bandwinidx = np.where(thrputfine[:,1]>=effimax*0.01)[0]
    bandl01 = thrputfine[bandwinidx[0],  0]
    bandr01 = thrputfine[bandwinidx[-1], 0]
    lambeff = np.sqrt(np.trapz(thrputfine[:,1], x=thrputfine[:,0], dx=10)/np.trapz(thrputfine[:,1]/thrputfine[:,0]**2, x=thrputfine[:,0], dx=10))
    return (bandl01,round(lambeff),bandr01)

def defbackposit():
    backsky = {}
    bandpos = {}
    for aBand in BandsCssEu:
        filterthr = np.loadtxt('./throughput/'+aBand+'.txt')
        backsky[aBand] = bskycalc(isky, filterthr, lambdarr)*PixScale[aBand]**2
        bandpos[aBand] = bandposit(filterthr)
    return backsky,bandpos

backsky,bandpos = defbackposit()
#print(backsky)
#print(bandpos)
...
def crs_solut(texp=texp0*nread0, tread=tread0*nread0, snr=snrlim, npix=npix80, bsky=0.1, bdet=0.02, bscat=scatlght, nread=nread0, rn=rn0):
...
def cr2mag(crs, hplk=hplk, cvl=cvlcm, band='g', aeff=TelArea['i'], ee=1.):
    xa = bandpos[band][0]
    xb = bandpos[band][2]
...
def mag2cr(mag, band='g', aeff=TelArea['i']):
...
def fnu2cr(fnu, hplk=hplk, band='g', aeff=TelArea['i']):
    xa = bandpos[band][0]
    xb = bandpos[band][2]
    essctel = ecsscntel(band, xa, xb)
    crs = fnu / hplk * essctel * aeff * math.log(bandpos[band][2] / bandpos[band][0])
...
def cr2fnu(crs, hplk=hplk, band='g', aeff=TelArea['i']):
    xa = bandpos[band][0]
    xb = bandpos[band][2]
    essctel = ecsscntel(band, xa, xb)
    fnu = crs / aeff * hplk / essctel / math.log(bandpos[band][2] / bandpos[band][0])
    return fnu


def mag2snr(mag, expcss, numb, band='g', aeff=TelArea['i']):
    ...
    snri = cr2snr(cr, texp=expcss, tread=tread0*numb, npix=aeff, bsky=backsky[band], bdet=bdet0, nread=numb, rn=rn0)
    # print snri
    return snri


def magpnt(band, bsky=0.1, texp=texp0*nread0, tread=tread0*nread0, nread=nread0, aeff=TelArea['i']):
    xa = bandpos[band][0]
    xb = bandpos[band][2]
    essctel = ecsscntel(band, xa, xb)
    # efflamb_1 = 0.2094
...
def magext(band, bsky=0.1, npixext=217, snr=10., lumfrac=0.9, aeff=TelArea['i']):
    xa = bandpos[band][0]
    xb = bandpos[band][2]
    essctel = ecsscntel(band, xa, xb)
...
class psfgauss():
    def __init__(self, ampli=1., sigma_x=1, sigma_y=1, theta=0, imgwidth = psfimgwidth):
        self.dx = dx
        self.ampli = ampli
...
def cnt2fnu(count, band, texp=300, aeff=TelArea['i']):
...
def ImgConvKnl(fwhmorig, fwhmgoal, pixscale=0.03, widthinfwhm=4):
    """
...
def pivot(bandcss):
    thrput = np.loadtxt(thrghdir+bandcss+'.txt')
    thrputfine = interp(thrput, xmin=bandpos[bandcss][0], xmax=bandpos[bandcss][2], dx=10)
    lambeff = np.sqrt(np.trapz(thrputfine[:,1], x=thrputfine[:,0], dx=10)/np.trapz(thrputfine[:,1]/thrputfine[:,0]**2, x=thrputfine[:,0], dx=10))
    return lambeff


def lbmean_leph(bandcss):
    '''
...
def NeFromSED(sedarr, bandcss, exptime, telarea, flamarr, debug=False):
    """
    ...
    thrput = np.loadtxt(thrghdir+bandcss+'.txt')
    sedfine = interp(sedarr, xmin=bandpos[bandcss][0], xmax=bandpos[bandcss][2], dx=1)
    thrputfine = interp(thrput, xmin=bandpos[bandcss][0], xmax=bandpos[bandcss][2], dx=1)
    obserspec = sedfine
    ...
    if debug==True:
        ...
        plt.annotate(bandcss, xy=(0.3,0.3), xycoords='figure fraction', color='k', horizontalalignment='center', fontSize=16)
        plt.xlim((2000,10000))
        plt.show()
    return Ne_obs


def Ne2MagAB(NeDet, bandcss, exptime, telarea):
    """
    ...
    # thrputdir = '/work/CSSOS/filter_improve/fromimg/windextract/throughput/'
    thrput = np.loadtxt(thrghdir+bandcss+'.txt')
    thrfine = interp(thrput, xmin=bandpos[bandcss][0], xmax=bandpos[bandcss][2], dx=1)
...
def Sed2Mag(sedarr, bandcss, magsim_zero):
    """
    ...
    :param bandcss: band name string;
...
def SedMag0(bandcss):
    ...
    Zero points can be obtained by running codes in ipython:
        ...
        > for aband in ['NUV', 'u', 'g', 'r', 'i', 'z', 'y', 'WU', 'WV', 'WI']:
        >     print(csstpkg.SedMag0(aband))
        >
    :param bandcss:
    :return: zero point of a bandcss
    """
    thrput = np.loadtxt(thrghdir+bandcss+'.txt')
    thrputfine = interp(thrput, xmin=bandpos[bandcss][0], xmax=bandpos[bandcss][2], dx=1)
...
#     Integ = np.trapz(obserspec[:,1], x=obserspec[:,0], dx=1.)
#     magsim = -2.5*math.log10(Integ) - MagSim_Zero[bandcss]
#     return magsim
...
def Ne2Fnu(NeDet, bandcss, exptime, telarea):
    """
...
def MagAB_Zero(Gain, bandcss, exptime, telarea):
    """
...
def FluxAdu_Zero(Gain, band, exptime, telarea):
    """
    ...
    """
    # thrputdir = '/work/CSSOS/filter_improve/fromimg/windextract/throughput/'
    thrput = np.loadtxt(thrghdir+band+'.txt')
    thrfine = interp(thrput, xmin=bandpos[band][0], xmax=bandpos[band][2], dx=1)
    ToBeInteg = thrfine[:,1]/thrfine[:,0]  # to be integrated
    Integ = np.trapz(ToBeInteg, x=thrfine[:,0], dx=1.)
    fnu0 = 1.0*Gain*hplk/exptime/telarea/Integ  # fnu in ergs/Hz/s/cm^2
    return fnu0
...
# def sed2magab_energy_zero(bandcss):
#     """
...
# def sed2magab_energy(modsed, bandcss, magzero):
#     thrput = np.loadtxt(thrghdir + bandcss + '.txt')
#     thrfine = interp(thrput, xmin=bandpos[bandcss][0], xmax=bandpos[bandcss][2], dx=1)
#     sedfine = interp(modsed, xmin=bandpos[bandcss][0], xmax=bandpos[bandcss][2], dx=1)
...
# def sed2magab_photon_zero(bandcss):
#     """
...
# def sed2magab_photon(modsed, bandcss, magzero):
#     thrput = np.loadtxt(thrghdir + bandcss + '.txt')
#     thrfine = interp(thrput, xmin=bandpos[bandcss][0], xmax=bandpos[bandcss][2], dx=1)
#     sedfine = interp(modsed, xmin=bandpos[bandcss][0], xmax=bandpos[bandcss][2], dx=1)
